...
const addPost = async () => {
    ...
    const author = profileUsername.textContent; // نستخدم الاسم من الملف الشخصي مباشرة
...
document.addEventListener('click', async (event) => {
    ...
    if (target.classList.contains('delete-btn')) {
        ...
        if (postDoc.exists()) {
            ...
            displayPosts();
            updateProfileInfo(); // تحديث عدد المنشورات بعد الحذف
        }
    }
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        displayPosts();
        updateProfileInfo(); // تحديث معلومات الملف الشخصي عند تسجيل الدخول
    } else {
        window.location.href = 'https://hussaindev10.github.io/Dhdhririeri/';
    }
});
